...
public class Node implements Piece {
	...
	public Node(GameBoardMitch gb, int player) {		
		this.gb = gb;
	...
	public Node(GameBoardMitch gb, int lastPlayer, Move lastMove) {
		this.gb = gb;
		this.childNodes = new ArrayList<Node>();
		this.utility = INVALID;
		...
		for (int i = 0; i < gb.n; i++) {
			for (int j = 0; j < gb.n; j++) {
				if (gb.board[i * gb.n + j] == EMPTY) {
					...
					GameBoardMitch childBoard = GameBoardMitch.newInstance(gb);
					childBoard.applyMove(placeMove);
					this.childNodes.add(new Node(childBoard, this.whosTurn, placeMove));
				}
			}
		}
	}
	...
	public void getChildJumpNodesFromPos(GameBoardMitch newBoard, int i, int j, ArrayList<Integer> rowList, ArrayList<Integer> colList) {
		...
		for (NeighbourPair pair : neighbourPairs) {
			...
			if (neighboursNeighbour == EMPTY && (neighbour == BLACK || neighbour == WHITE)) {
				...
				Node childNode = new Node(childBoard, this.whosTurn, jumpMove);
	...
	public static ArrayList<Node> getTrainingSet(int size, int n, int player) {
		...
		for (int i = 0; i < size; i++) {
			Node node = new Node(GameBoardMitch.getRandomBoard(n), player);
