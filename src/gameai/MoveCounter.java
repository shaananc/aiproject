...
/* MoveCounter objects are used to count the number of legal
   moves possible for a given player on a given GameBoardMitch
*/

public class MoveCounter {

	private int n;					// dimension of board
	private Map<GameBoardMitch, Integer> moveCache;		// store GameBoardMitch/numMoves entries for lookup
	
	public MoveCounter(int n) {
		this.n = n;
		this.moveCache = new HashMap<GameBoardMitch, Integer>();
	}
	
	// count number of possible moves "player" could make on GameBoardMitch "gb"
	public int countMoves(GameBoardMitch gb, Cell.State player) {
	...
	public int countPlaceMoves(GameBoardMitch gb) {
		int placeMoves = 0;
	...
	// count number of jump moves "player" could make from GameBoardMitch "gb"
	public int countJumpMoves(GameBoardMitch gb, Cell.State player) {
		int jumpMoves = 0;
	...
	// in GameBoardMitch "gb"
	public int countJumpMovesFromPos(GameBoardMitch gb, Cell.State player, Pos pos, int depth) {
		// DEBUGGING
		/*
		System.out.println("in countJumpMovesFromPos at pos (" + pos.i + ", " + pos.j + ")");
		System.out.println("depth = " + depth);
		System.out.println("GameBoardMitch:");
		...
		for (NeighbourPair neighbourPair : neighbourPairs) {
			...
			if (gb.board[ni*n + nj].isOccupied()) {
				if (gb.board[nni*n + nnj].state == Cell.State.E) {
					...
					GameBoardMitch child = gb.executeMove(player, new Pos(nni, nnj), new Pos(ni, nj));
